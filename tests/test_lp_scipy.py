...
# NOTE: you're repeating this code in multiple places, consider refactoring into a common utility function or pull from dataset instead
def _get_binary_variables(output_path: str):
    ...
    return is_on.clamp_min(0).T, is_charging.clamp_min(0).T, is_discharging.clamp_min(0).T
...
explanatory_vars_path = "data/Train_Data/instance_2021_Q1_1/explanatory_variables.xlsx"
...
solution = EDModelLP(instance_path=sample_path)(
    load, solar_max, wind_max, is_on, is_charging, is_discharging
)

with open("tests/ed_model_lp_solution.pkl", "wb") as f:
