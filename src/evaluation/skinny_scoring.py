...
def get_input_data():
    ...
    for instance in list_ins:
        ...
        with gzip.open(reference_dir+instance+"/InputData.json.gz","r") as f:
            ...
            data_in[index] = json.loads(in_json)
...
def main():
    ...
    data_in, i_done = get_input_data()
    print('-' * 10)
    print('Reading Reference Data')
    prediction = get_prediction()
    print('-' * 10)
    meta = pd.read_csv("instances_actual_data.csv",index_col=0)
    score = pd.DataFrame()
    # Duration training
    ...
    for case in data_in.keys():
        for g in data_in[case]["Generators"].keys():
            if data_in[case]["Generators"][g]["Type"] == "Thermal":
                data_in[case]["Generators"][g]["Commitment status"] = [int(round(item)) for item in prediction[case][g].tolist()]
        ...
        print(f'instance solved {case}')

        real_value = meta[meta.i_done==i_done[case]]["objetive function ($)"].tolist()[0]
        real_time  = meta[meta.i_done==i_done[case]]["run time (s)"].tolist()[0]
